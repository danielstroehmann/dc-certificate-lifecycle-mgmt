#!/bin/bash
# Enroll a DigiCert TLM certificate into a YubiKey PIV slot via EST (Linux).
# Subject: CN plus email SAN (rfc822Name), for profiles backed by a Microsoft CA.
# All secrets and parameters come from ./.env (create it with ./config-linux.sh).
...
[[ -f "$ENV_FILE" ]] || { echo "ERROR: $ENV_FILE not found. Run ./config-linux.sh first."; exit 1; }
# shellcheck source=/dev/null
source "$ENV_FILE"

for var in EST_BASE_URL EST_PROFILE_ID_MSCA ENROLL_CODE CN EMAIL SLOT PIN MGMT_KEY PKCS11_LIB PKCS11_ENGINE; do
  [[ -n "${!var:-}" ]] || { echo "ERROR: $var is not set in $ENV_FILE"; exit 1; }
done
EST_URL="$EST_BASE_URL/$EST_PROFILE_ID_MSCA/simpleenroll"

[[ -f "$PKCS11_LIB" ]] || { echo "ERROR: $PKCS11_LIB not found. Fix PKCS11_LIB in $ENV_FILE."; exit 1; }
[[ -f "$PKCS11_ENGINE" ]] || { echo "ERROR: $PKCS11_ENGINE not found. Run: sudo apt install libengine-pkcs11-openssl"; exit 1; }
...
openssl req -in "$TMPDIR/csr.pem" -noout -text | grep -E "(Subject:|email:)"

# --- Submit to EST and import the certificate --------------------------------
PASSCODE_B64=$(printf '%s' "$ENROLL_CODE" | base64 -w0)
...
base64 -d "$TMPDIR/response.b64" > "$TMPDIR/response.p7.der"
